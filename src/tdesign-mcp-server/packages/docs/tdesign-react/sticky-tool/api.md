...
popupProps | Object | - | 透传 Popup 组件全部特性，优先级低于 StickyItem.popupProps。TS 类型：`PopupProps`，[Popup API Documents](./popup?tab=api)。 | N
...
label | TNode | - | 名称。TS 类型：`string \| TNode`。 | N
popup | TNode | - | 浮层内容。TS 类型：`string \| TNode`。 | N
popupProps | Object | - | 透传浮层组件全部属性。TS 类型：`PopupProps`，[Popup API Documents](./popup?tab=api)。 | N
