...
tableLayout | String | auto | 用于设置底层 `table` 单元格、行和列的布局算法，与原生 table-layout css 属性完全一致。`fixed`：采用固定布局算法；`auto`：采用自动布局算法。详情可参考 。可选项：fixed/auto | N
title | TNode | - | 描述列表的标题。TS 类型：`string \| TNode`。 | N
...
className | String | - | 类名 | N
style | Object | - | 样式，TS 类型：`React.CSSProperties` | N
content | TNode | - | 描述项内容。TS 类型：`string \| TNode`。 | N
label | TNode | - | 描述项标签。TS 类型：`string \| TNode`。 | N
