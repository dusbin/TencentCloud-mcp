...
label | TNode | - | 左侧文本。TS 类型：`string \| TNode`。 | N
...
prefixIcon | TElement | - | 用于自定义组件前置图标。TS 类型：`TNode`。 | N
...
suffixIcon | TElement | - | 用于自定义组件后置图标。TS 类型：`TNode`。 | N
...
value | String / Number / Array / Date | '' | 选中值。TS 类型：`DateValue \| DateMultipleValue` ` type DateValue = string \| number \| Date ` ` type DateMultipleValue = Array<DateValue> `。 | N
...
onChange | Function |  | TS 类型：`(value: DateValue, context: { dayjsValue?: Dayjs, trigger?: DatePickerTriggerSource }) => void`<br/>选中值发生变化时触发。。<br/>`import { Dayjs } from 'dayjs'`<br/><br/>`type DatePickerTriggerSource = 'confirm' \| 'pick' \| 'enter' \| 'preset' \| 'clear' \| 'tag-remove'`<br/> | N
...
popupProps | Object | - | 透传给 popup 组件的参数。TS 类型：`PopupProps`，[Popup API Documents](./popup?tab=api)。 | N
prefixIcon | TElement | - | 组件前置图标。TS 类型：`TNode`。 | N
...
size | String | medium | 输入框尺寸。可选项：small/medium/large。TS 类型：`SizeEnum`。 | N
status | String | default | 输入框状态。可选项：default/success/warning/error | N
suffixIcon | TElement | - | 组件后置图标。TS 类型：`TNode`。 | N
timePickerProps | Object | - | 透传 TimePicker 组件属性。TS 类型：`TimePickerProps`，[TimePicker API Documents](./time-picker?tab=api)。 | N
tips | TNode | - | 输入框下方提示文本，会根据不同的 `status` 呈现不同的样式。TS 类型：`string \| TNode`。 | N
