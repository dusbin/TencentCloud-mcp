...
logo | TElement | - | 站点 LOGO。TS 类型：`TNode`。 | N
operations | TElement | - | 导航操作区域。TS 类型：`TNode`。 | N
...
value | String / Number | - | 激活菜单项。TS 类型：`MenuValue` `type MenuValue = string \| number`。 | N
...
className | String | - | 类名 | N
style | Object | - | 样式，TS 类型：`React.CSSProperties` | N
children | TNode | - | 菜单项内容，同 content。TS 类型：`string \| TNode`。 | N
content | TNode | - | 菜单项内容。TS 类型：`string \| TNode`。 | N
disabled | Boolean | - | 是否禁用菜单项展开/收起/跳转等功能 | N
icon | TElement | - | 菜单项图标。TS 类型：`TNode`。 | N
popupProps | Object | - | 透传 Popup 组件全部特性。TS 类型：`PopupProps`，[Popup API Documents](./popup?tab=api)。 | N
title | TNode | - | 二级菜单内容。TS 类型：`string \| TNode`。 | N
...
disabled | Boolean | - | 是否禁用菜单项展开/收起/跳转等功能 | N
href | String | - | 跳转链接 | N
icon | TElement | - | 图标。TS 类型：`TNode`。 | N
target | String | - | 链接或路由跳转方式。可选项：_blank/_self/_parent/_top | N
tooltipProps | Object | - | 透传 Tooltip 组件的特性，作用于一级菜单收起下聚焦时出现的节点。TS 类型：`TooltipProps`，[Tooltip API Documents](./tooltip?tab=api)。 | N
value | String / Number | - | 菜单项唯一标识。TS 类型：`MenuValue` | N
...
className | String | - | 类名 | N
style | Object | - | 样式，TS 类型：`React.CSSProperties` | N
title | TNode | - | 菜单组标题。TS 类型：`string \| TNode`。 | N