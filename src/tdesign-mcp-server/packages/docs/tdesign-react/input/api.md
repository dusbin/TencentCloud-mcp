...
format | Function | - | 指定输入框展示值的格式。TS 类型：`InputFormatType` `type InputFormatType = (value: InputValue) => string`。 | N
...
value | String | '' | 输入框的值。TS 类型：`InputValue` `type InputValue = string`。 | N
defaultValue | String | '' | 输入框的值。非受控属性。TS 类型：`InputValue` `type InputValue = string`。 | N
