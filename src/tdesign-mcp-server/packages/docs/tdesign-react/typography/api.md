...
children | TNode | - | 文本内容，同content。TS 类型：`string \| TNode`。 | N
...
className | String | - | 类名 | N
style | Object | - | 样式，TS 类型：`React.CSSProperties` | N
children | TNode | - | 段落内容，同 content。TS 类型：`string \| TNode`。 | N
content | TNode | - | 段落内容。TS 类型：`string \| TNode`。 | N
ellipsis | Boolean / Object | false | 是否省略展示，可通过配置参数自定义省略操作的具体功能和样式。TS 类型：`boolean \| TypographyEllipsis` | N
...
suffix | TElement | - | 自定义省略触发元素，一般用于自定义折叠图标。TS 类型：`TNode<{ expanded: boolean }>`。 | N
...
suffix | TElement | - | 自定义复制触发元素，一般用于自定义复制图标。TS 类型：`TNode<{ copied: boolean }>`。 | N
