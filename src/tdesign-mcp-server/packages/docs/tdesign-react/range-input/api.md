...
label | TNode | - | 左侧内容。TS 类型：`string \| TNode`。 | N
...
separator | TNode | '-' | 范围分隔符。TS 类型：`string \| TNode`。 | N
...
value | Array | [] | 范围输入框的值。TS 类型：`RangeInputValue` `type RangeInputValue = Array<InputValue>`。 | N
