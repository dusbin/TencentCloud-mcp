...
className | String | - | 类名 | N
style | Object | - | 样式，TS 类型：`React.CSSProperties` | N
children | TNode | - | 描述内容，同 content。TS 类型：`string \| TNode`。 | N
content | TNode | - | 描述内容。TS 类型：`string \| TNode`。 | N
dot | TElement | - | 用于自定义时间轴节点元素。TS 类型：`TNode`。 | N
dotColor | String | primary | 时间轴颜色，内置 `primary/warning/error/default` 四种色值，可传入 16 进制颜色码或 RGB 颜色值.。TS 类型：`string` | N
label | TNode | - | 标签文本内容，可完全自定义。TS 类型：`string \| TNode`。 | N
