...
children | TNode | - | 回到顶部内容，同 `content`。TS 类型：`string \| TNode`。 | N
container | String / Function | 'body' | 监听滚动的容器。数据类型为 String 时，会被当作选择器处理，进行节点查询。示例：'body' 或 () => document.body。TS 类型：`AttachNode`。 | N
content | TNode | - | 回到顶部内容。TS 类型：`string \| TNode`。 | N
default | TNode | - | 回到顶部内容，同 `content`。TS 类型：`string \| TNode`。 | N
...
target | String / Function | 'body' | 指定回到该对象。数据类型为 String 时，会被当作选择器处理，进行节点查询。示例：'body' 或 () => document.body。TS 类型：`AttachNode`。 | N
