...
asyncLoading | TNode | - | 自定义加载中。值为空不显示加载中，值为 'loading' 显示加载中状态，值为 'load-more' 显示加载更多状态。值类型为函数，则表示自定义加载状态呈现内容。TS 类型：`string \| TNode`。 | N
footer | TNode | - | 底部。TS 类型：`string \| TNode`。 | N
header | TNode | - | 头部。TS 类型：`string \| TNode`。 | N
...
className | String | - | 类名 | N
style | Object | - | 样式，TS 类型：`React.CSSProperties` | N
action | TNode | - | 操作栏。TS 类型：`string \| TNode`。 | N
children | TNode | - | 内容，同 content。TS 类型：`string \| TNode`。 | N
...
className | String | - | 类名 | N
style | Object | - | 样式，TS 类型：`React.CSSProperties` | N
description | TNode | - | 列表项内容。TS 类型：`string \| TNode`。 | N
image | TNode | - | 列表项图片。TS 类型：`string \| TNode`。 | N
title | TNode | - | 列表项标题。TS 类型：`string \| TNode`。 | N