...
children | TNode | - | 子元素内容，同 content。TS 类型：`string \| TNode`。 | N
content | TNode | - | 子元素内容。TS 类型：`string \| TNode`。 | N
...
collapseAvatar | TNode | - | 头像数量超出时，会出现一个头像折叠元素。该元素内容可自定义。默认为 `+N`。示例：`+5`，`...`, `更多`。TS 类型：`string \| TNode`。 | N
