...
affixProps | Object | - | 透传 Affix 组件属性，即让 Anchor 组件支持所有 Affix 组件特性。TS 类型：`Omit<AffixProps, 'children'>`，[Affix API Documents](./affix?tab=api)。 | N
...
cursor | TElement | - | 用于自定义选中项左侧游标。TS 类型：`TNode`。 | N
...
title | TNode | '' | 锚点文本。TS 类型：`string \| TNode`。 | N
