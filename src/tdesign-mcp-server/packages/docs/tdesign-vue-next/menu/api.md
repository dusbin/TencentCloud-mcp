...
logo | Slot / Function | - | 站点 LOGO。TS 类型：`TNode`。 | N
operations | Slot / Function | - | 导航操作区域。TS 类型：`TNode`。 | N
...
value | String / Number | - | 激活菜单项。支持语法糖 `v-model` 或 `v-model:value`。TS 类型：`MenuValue` `type MenuValue = string \| number`。 | N
defaultValue | String / Number | - | 激活菜单项。非受控属性。TS 类型：`MenuValue` `type MenuValue = string \| number`。 | N
...
名称 | 类型 | 默认值 | 说明 | 必传
-- | -- | -- | -- | --
content | String / Slot / Function | - | 菜单项内容。TS 类型：`string \| TNode`。 | N
default | String / Slot / Function | - | 菜单项内容，同 content。TS 类型：`string \| TNode`。 | N
disabled | Boolean | - | 是否禁用菜单项展开/收起/跳转等功能 | N
icon | Slot / Function | - | 菜单项图标。TS 类型：`TNode`。 | N
popupProps | Object | - | 透传 Popup 组件全部特性。TS 类型：`PopupProps`，[Popup API Documents](./popup?tab=api)。 | N
title | String / Slot / Function | - | 二级菜单内容。TS 类型：`string \| TNode`。 | N
...
to | String / Object | - | 路由跳转目标，当且仅当 Router 存在时，该 API 有效。TS 类型：`MenuRoute` `interface MenuRoute { path?: string; name?: string; hash?: string; query?: MenuQueryData; params?: MenuQueryData }` `type MenuQueryData = { [key: string]: string \| string[] }`。 | N
value | String / Number | - | 菜单项唯一标识。TS 类型：`MenuValue` | N
...
名称 | 类型 | 默认值 | 说明 | 必传
-- | -- | -- | -- | --
title | String / Slot / Function | - | 菜单组标题。TS 类型：`string \| TNode`。 | N