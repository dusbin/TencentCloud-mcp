...
checkProps | Object | - | 透传属性到 checkbox 组件。参考 checkbox 组件 API。TS 类型：`CheckboxProps`，[Checkbox API Documents](./checkbox?tab=api)。 | N
...
empty | String / Slot / Function | '' | 数据为空时展示的文本。TS 类型：`string \| TNode`。 | N
...
icon | Boolean / Slot / Function | true | 节点图标，可自定义。TS 类型：`boolean \| TNode<TreeNodeModel<T>>`。 | N
...
label | String / Boolean / Slot / Function | true | 自定义节点内容，值为 `false` 不显示，值为 `true` 显示默认 label，值为字符串直接输出该字符串。泛型 `T` 表示树节点 TS 类型。<br/>如果期望只有点击复选框才选中，而点击节点不选中，可以使用 `label` 自定义节点，然后加上点击事件 `e.preventDefault()`，通过调整自定义节点的宽度和高度决定禁止点击选中的范围。TS 类型：`string \| boolean \| TNode<TreeNodeModel<T>>`。 | N
lazy | Boolean | true | 延迟加载 children 为 true 的节点的子节点数据，即使 expandAll 被设置为 true，也同样延迟加载 | N
line | Boolean / Slot / Function | false | 连接线。值为 false 不显示连接线；值为 true 显示默认连接线；值类型为 Function 表示自定义连接线。TS 类型：`boolean \| TNode`。 | N
...
operations | Slot / Function | - | 自定义节点操作项，泛型 `T` 表示树节点 TS 类型。TS 类型：`TNode<TreeNodeModel<T>>`。 | N
...
value | Array | [] | 选中值，组件为可选状态时有效。支持语法糖 `v-model` 或 `v-model:value`。TS 类型：`Array<TreeNodeValue>` `type TreeNodeValue = string \| number`。 | N
defaultValue | Array | [] | 选中值，组件为可选状态时有效。非受控属性。TS 类型：`Array<TreeNodeValue>` `type TreeNodeValue = string \| number`。 | N
