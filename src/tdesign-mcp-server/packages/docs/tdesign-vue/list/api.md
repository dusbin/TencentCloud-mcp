...
名称 | 类型 | 默认值 | 描述 | 必传
-- | -- | -- | -- | --
asyncLoading | String / Slot / Function | - | 自定义加载中。值为空不显示加载中，值为 'loading' 显示加载中状态，值为 'load-more' 显示加载更多状态。值类型为函数，则表示自定义加载状态呈现内容。TS 类型：`string \| TNode`。 | N
footer | String / Slot / Function | - | 底部。TS 类型：`string \| TNode`。 | N
header | String / Slot / Function | - | 头部。TS 类型：`string \| TNode`。 | N
...
名称 | 类型 | 默认值 | 描述 | 必传
-- | -- | -- | -- | --
action | String / Slot / Function | - | 操作栏。TS 类型：`string \| TNode`。 | N
...
名称 | 类型 | 默认值 | 描述 | 必传
-- | -- | -- | -- | --
avatar | String / Slot / Function | - | 已废弃。列表项图片。TS 类型：`string \| TNode`。 | N
description | String / Slot / Function | - | 列表项内容。TS 类型：`string \| TNode`。 | N
image | String / Slot / Function | - | 列表项图片。TS 类型：`string \| TNode`。 | N
title | String / Slot / Function | - | 列表项标题。TS 类型：`string \| TNode`。 | N