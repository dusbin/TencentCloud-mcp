...
keys | Object | - | 用来定义 `value / label / disabled / children` 在 `data` 数据中对应的字段别名，示例：`{ value: 'key', label 'name', children: 'list' }`。其中，disabled 待开发。。TS 类型：`TreeKeysType`。 | N
...
value | Array | [] | 选中值，组件为可选状态时有效。支持语法糖 `v-model`。TS 类型：`Array<TreeNodeValue>` `type TreeNodeValue = string \| number`。 | N
